...
function addPart() {
  ...
  newLabel.className = "model-label";
  newLabel.innerText = "Select Part:";

  const newRow = document.createElement("div");
  newRow.className = "parts-row";

  newRow.innerHTML = `
    <select id="parts-dropdown-${Date.now()}" class="parts-dropdown" onchange="updateResults()">
    ...
    <button class="remove-part-btn" onclick="removePart(this)">❌</button>
  `;

  newLabel.htmlFor = newRow.querySelector(".parts-dropdown").id;
  newRow.prepend(newLabel);

  document.querySelector(".parts-list-container").appendChild(newRow);
  updateResults();
}
      ...
      if (parts) {
        parts.split(", ").forEach((partQuantity) => {
          ...
          const lastRow = document.querySelectorAll(
            ".parts-list-container .parts-row:last-child"
          )[0];
